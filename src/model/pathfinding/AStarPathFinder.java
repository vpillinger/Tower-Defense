...
public class AStarPathFinder implements PathFinder, PathFindingContext {
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x=-1;x<2;x++) {
				for (int y=-1;y<2;y++) {
					...
					if(xp < 0){
						xp = map.getWidthInTiles() - 1;
					}
					if(xp > map.getWidthInTiles() - 1){
						xp = 0;
					}
					if(yp < 0){
						yp = map.getHeightInTiles() - 1;
					}
					if(yp > map.getHeightInTiles() - 1){
						yp = 0;
					}
	...
	public ArrayList<Point> getClosed() {
		ArrayList<Point> c = new ArrayList<Point>(closed.size());
		for(Node x : closed){
			c.add(new Point(x.x, x.y));
		}
		return c;
	}
	public ArrayList saveClosed(){
		ArrayList<Node> cl = new ArrayList<Node>(closed.size());
		for(Node n : closed){
			cl.add(new Node(n.x,n.y));
		}
		return cl;
	}
	public void setClosed(ArrayList c){
		closed = c;
	}
}
